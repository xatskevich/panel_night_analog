...
//ШИМ кнопок
	if (pwm_count >= butt_b[0]) reset_green_led_power; //питание
	else set_green_led_power;
	if (pwm_count >= butt_b[1]) reset_green_led_clutch; //сцепление
	else set_green_led_clutch;
	if (pwm_count >= butt_b[2]) reset_green_led_left; //осв лев
	else set_green_led_left;
	if (pwm_count >= butt_b[3]) reset_green_led_right; //осв прав
